...
def get_maturity_rating(entry):
    """Extract maturity rating from panel.episode_metadata.maturity_ratings"""
    try:
        maturity = entry["panel"]["episode_metadata"]["maturity_ratings"]

        if isinstance(maturity, str):
            return maturity.strip()
        elif isinstance(maturity, list):
            return str(maturity[0]).strip() if maturity else "Unknown"
        elif isinstance(maturity, dict):
            return maturity.get("rating") or maturity.get("level") or str(maturity)
    ...
    # Old fallback
    try:
        return entry.get("extended_maturity_rating", {}).get("rating", "Unknown")
    except:
        pass
...
def get_content_descriptors(entry):
    """Extract content_descriptors from panel.episode_metadata.content_descriptors"""
    try:
        descriptors = entry["panel"]["episode_metadata"]["content_descriptors"]
        if isinstance(descriptors, list):
            return [str(d).strip() for d in descriptors if str(d).strip()]
    except (KeyError, TypeError):
        pass

    # Root level fallback
    try:
        descriptors = entry.get("content_descriptors")
        if isinstance(descriptors, list):
            return [str(d).strip() for d in descriptors if str(d).strip()]
    except:
        pass
...
def main():
    ...
    for series_id, episodes in series_groups.items():
        ...
        genres = get_genres(first_ep)
        maturity_rating = get_maturity_rating(first_ep)
        content_descriptors = get_content_descriptors(first_ep)
        ...
        report.append({
            ...
            "content_descriptors": content_descriptors,  # ← Now from panel.episode_metadata.content_descriptors
    ...
    if genre_scores:
        ...
        print("\nℹ️  No genre/category data found.")

    print("\n🏆 TOP 15 HIGHEST SCORED SERIES")
    for i, r in enumerate(report[:15], 1):
        genres_str = ", ".join(r.get("genres", []))[:90]
        descriptors_str = ", ".join(r.get("content_descriptors", []))[:100]
        ...
        print(f"    Maturity  : {r['maturity_rating']}")
        if genres_str:
            print(f"    Genres    : {genres_str}")
        if descriptors_str:
            print(f"    Descriptors: {descriptors_str}")
